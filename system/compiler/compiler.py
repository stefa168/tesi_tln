...
def prepare_dataset(df: DataFrame,
                    ...
                    data_column: str,
                    label_column: str) -> tuple[Dataset, Dataset]:
    """
    ...
    Args:
        ...
        data_column (str): The name of the column containing the text data.
    ...
    train_index, val_index = next(split.split(df, df[label_column]))
...
def main():
    ...
    label_column = "Global Subject"
    label_info = LabelInfo(df, label_column)

    model_name = 'bert-base-uncased'

    tokenizer = AutoTokenizer.from_pretrained(model_name)

    train_dataset, eval_dataset = prepare_dataset(df, tokenizer, label_info, data_column, label_column)

    model = prepare_model(model_name, label_info)
